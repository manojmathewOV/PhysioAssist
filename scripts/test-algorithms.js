...
test('Angle smoothing reduces variance', () => {
  ...
  assert(
    smoothed.every((a) => a >= 85 && a <= 95),
    'Smoothed values in range'
  );
...
test('Detect repetition completion', () => {
  const angleHistory = [170, 165, 140, 100, 60, 40, 45, 80, 120, 160, 175];
...
function isPoseStable(current, previous, threshold) {
  ...
  for (let i = 0; i < current.length; i++) {
    if (
      current[i] &&
      previous[i] &&
      current[i].visibility > 0.5 &&
      previous[i].visibility > 0.5
    ) {
