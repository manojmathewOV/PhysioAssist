...
describe('progress as one series per exercise', () => {
  ...
  it('never mixes exercises, keeps unmeasured sessions, and compares within one plan', () => {
    ...
    expect(active.latest?.degrees).toBe(8);
    expect(active.firstThisPlan?.degrees).toBe(12); // not 25, from the old plan
  ...
  it('Progress shows the series without claiming improvement', () => {
    const { getByTestId, queryByText } = render(
      <Provider store={storeWith(kneePlan, history)}>
        <ProgressScreen />
      </Provider>
    );
    expect(getByTestId('progress-series')).toHaveTextContent(/Latest 8° from straight/);
    expect(getByTestId('progress-series')).toHaveTextContent(
      /First with this plan 12° from straight/
    );
    expect(getByTestId('progress-series')).toHaveTextContent(
