...
export class SyntheticPoseDataGenerator {
  ...
  public generateShoulderFlexion(
    ...
    // Sagittal plane movement: no lateral (X) component, only vertical (Y) and anterior-posterior (Z)
    // Z+ is toward camera (anterior), so forward flexion increases Z
    ...
    const wrist: Vector3D = {
      x: elbow.x, // No lateral movement
      y: elbow.y - forearmLength * Math.sin(forearmAngleFromHorizontalRad), // Vertical component
      z: elbow.z + forearmLength * Math.cos(forearmAngleFromHorizontalRad), // Anterior component (+ for toward camera)
    };
  ...
  public generateElbowFlexion(
    ...
    // Clinical: 0° = straight arm (wrist below elbow), 150° = fully flexed (wrist near shoulder)
    // Y+ is downward, so: 0° means wrist has larger Y (below), 90° means same Y (forward in Z)
    // Sagittal plane movement: no lateral (X) component, only vertical (Y) and anterior-posterior (Z)
    // Z+ is toward camera (anterior in frontal view), Z- is away from camera (posterior)
    const flexionRad = (angle * Math.PI) / 180;
    const wrist: Vector3D = {
      x: elbow.x, // No lateral movement
      y: elbow.y + forearmLength * Math.cos(flexionRad), // Vertical component (+ for down)
      z: elbow.z + forearmLength * Math.sin(flexionRad), // Anterior component (+ for toward camera)
    };
  ...
  public generateKneeFlexion(
    ...
    // Sagittal plane movement: no lateral (X) component, only vertical (Y) and anterior-posterior (Z)
    // For knee flexion, foot moves backward (away from camera, Z-)
    const flexionRad = (angle * Math.PI) / 180;
    const ankle: Vector3D = {
      x: knee.x, // No lateral movement
      y: knee.y + shinLength * Math.cos(flexionRad), // Vertical component (+ for down)
      z: knee.z - shinLength * Math.sin(flexionRad), // Posterior component (- for away from camera)
    };
  ...
  public generateShoulderRotation(
    ...
    schemaId: 'movenet-17' | 'mediapipe-33' = 'movenet-17',
    options: {
      elbowAngle?: number; // Should be 90° for valid rotation measurement
      side?: 'left' | 'right';
      viewOrientation?: 'frontal' | 'sagittal';
    } = {}
  ): { poseData: ProcessedPoseData; groundTruth: GroundTruth } {
    const { elbowAngle = 90, side = 'right', viewOrientation = 'frontal' } = options;
    ...
    // Wrist position with specified elbow angle and rotation
    // Goniometer measures interior angle at elbow joint
    // Interior angle = angle between (elbow->shoulder) and (elbow->wrist) vectors

    const rotationRad = (angle * Math.PI) / 180;
    const elbowAngleRad = (elbowAngle * Math.PI) / 180;

    // Reference direction: elbow back toward shoulder
    // This is opposite of humerus direction (shoulder->elbow)
    // const elbowToShoulder = { x: -lateralMultiplier, y: 0, z: 0 };

    // Forearm direction (elbow->wrist) should make angle 'elbowAngle' with elbow-to-shoulder direction
    // Use rotation angle to determine orientation in YZ plane perpendicular to humerus
    //
    // Interior angle decomposition:
    // - X component aligns with elbowToShoulder at angle elbowAngle: -cos(elbowAngle) * lateralMultiplier
    // - YZ components form perpendicular part: sin(elbowAngle) in direction determined by rotation
    //
    // At rotation=0° and elbowAngle=90°: forearm points straight up (negative Y)
    // At rotation=90° and elbowAngle=90°: forearm points forward (positive Z)

    const forearmDirX = -Math.cos(elbowAngleRad) * lateralMultiplier;
    const forearmDirY = -Math.sin(elbowAngleRad) * Math.cos(rotationRad); // Negative Y is up
    const forearmDirZ = Math.sin(elbowAngleRad) * Math.sin(rotationRad);

    const wrist: Vector3D = {
      x: elbow.x + forearmLength * forearmDirX,
      y: elbow.y + forearmLength * forearmDirY,
      z: elbow.z + forearmLength * forearmDirZ,
    };
    ...
    const groundTruth: GroundTruth = {
      primaryMeasurement: {
        joint: `${side}_shoulder`,
        angle: Math.abs(angle), // Store absolute value for comparison
        plane: 'transverse',
        movement: angle >= 0 ? 'external_rotation' : 'internal_rotation',
      },
      secondaryMeasurements: [
        {
          joint: `${side}_elbow`,
          angle: elbowAngle,
          expectedDeviation: Math.abs(elbowAngle - 90), // Should be at 90° for valid measurement
        },
      ],
      compensations: [],
