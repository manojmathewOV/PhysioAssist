...
export class ValidationPipeline {
  ...
  private async validateShoulderFlexion(): Promise<{
    ...
    for (const side of sides) {
      for (const angle of testAngles) {
        ...
        const measurement = this.createMeasurementService().measureShoulderFlexion(
          enrichedPose,
          side
        );
        ...
        if (angle >= 90) {
          ...
          const compensatedMeasurement =
            this.createMeasurementService().measureShoulderFlexion(
  ...
  private async validateShoulderAbduction(): Promise<{
    ...
    for (const side of sides) {
      for (const angle of testAngles) {
        ...
        const enrichedPose = this.addAnatomicalFrames(poseData);
        const measurement = this.createMeasurementService().measureShoulderAbduction(
          enrichedPose,
          side
        );
        ...
        if (angle >= 90) {
          ...
          const enrichedCompensatedPose = this.addAnatomicalFrames(compensatedPose);
          const compensatedMeasurement =
            this.createMeasurementService().measureShoulderAbduction(
              enrichedCompensatedPose,
              side
            );
  ...
  private async validateShoulderRotation(): Promise<{
    ...
    for (const side of sides) {
      for (const angle of testAngles) {
        ...
        const enrichedPose = this.addAnatomicalFrames(poseData);
        const measurement = this.createMeasurementService().measureShoulderRotation(
          enrichedPose,
          side
        );
  ...
  private async validateElbowFlexion(): Promise<{
    ...
    for (const side of sides) {
      for (const angle of testAngles) {
        ...
        const enrichedPose = this.addAnatomicalFrames(poseData);
        const measurement = this.createMeasurementService().measureElbowFlexion(
          enrichedPose,
          side
        );
  ...
  private async validateKneeFlexion(): Promise<{
    ...
    for (const side of sides) {
      for (const angle of testAngles) {
        ...
        const enrichedPose = this.addAnatomicalFrames(poseData);
        const measurement = this.createMeasurementService().measureKneeFlexion(
          enrichedPose,
          side
        );
  ...
  private async validateEdgeCases(): Promise<{
    ...
    for (const test of nearZeroTests) {
      ...
      let measurement;
      if (test.movement === 'shoulder_flexion') {
        measurement = this.createMeasurementService().measureShoulderFlexion(
          enrichedPose,
          test.side
        );
      } else if (test.movement === 'shoulder_abduction') {
        measurement = this.createMeasurementService().measureShoulderAbduction(
          enrichedPose,
          test.side
        );
      } else if (test.movement === 'elbow_flexion') {
        measurement = this.createMeasurementService().measureElbowFlexion(
          enrichedPose,
          test.side
        );
      } else {
        measurement = this.createMeasurementService().measureKneeFlexion(
          enrichedPose,
          test.side
        );
      }
    ...
    for (const angle of bilateralAngles) {
      ...
      const leftMeasurement = this.createMeasurementService().measureShoulderFlexion(
        enrichedLeftPose,
        'left'
      );
      ...
      const rightMeasurement = this.createMeasurementService().measureShoulderFlexion(
        enrichedRightPose,
        'right'
      );
